        ...
        if (event.phase == TickEvent.Phase.END && event.player instanceof ServerPlayerEntity) {
            ...
            if (configAllows && (hasRadar || hasHelmet)) {
                ...
                if (serverPlayer.world.getDayTime() % 24000 == 40) {
                    ...
                    float tomorrowMorningTemp = Math.round(ClimateData.getFutureTemp(serverPlayer.world, 1, 0) * 10) / 10.0F;
                    serverPlayer.sendStatusMessage(GuiUtils.translateMessage("forecast.morning",
                            morningTemp, noonTemp, nightTemp, midnightTemp, tomorrowMorningTemp), false);
                    boolean snow =
                            morningTemp < WorldClimate.SNOW_TEMPERATURE ||
                                    noonTemp < WorldClimate.SNOW_TEMPERATURE ||
                                    nightTemp < WorldClimate.SNOW_TEMPERATURE ||
                                    midnightTemp < WorldClimate.SNOW_TEMPERATURE ||
                                    tomorrowMorningTemp < WorldClimate.SNOW_TEMPERATURE;
                    boolean blizzard =
                            morningTemp < WorldClimate.BLIZZARD_TEMPERATURE ||
                ...
                if (serverPlayer.world.getDayTime() % 24000 == 12542) {
                    ...
                    float tomorrowMorningTemp = Math.round(ClimateData.getFutureTemp(serverPlayer.world, 0, 12) * 10) / 10.0F;
                    float tomorrowNoonTemp = Math.round(ClimateData.getFutureTemp(serverPlayer.world, 0, 18) * 10) / 10.0F;
                    float tomorrowNightTemp = Math.round(ClimateData.getFutureTemp(serverPlayer.world, 1, 0) * 10) / 10.0F;
                    serverPlayer.sendStatusMessage(GuiUtils.translateMessage("forecast.night",
                            nightTemp, midnightTemp, tomorrowMorningTemp, tomorrowNoonTemp, tomorrowNightTemp), false);
                    boolean snow =
                            nightTemp < WorldClimate.SNOW_TEMPERATURE ||
                                    midnightTemp < WorldClimate.SNOW_TEMPERATURE ||
                                    tomorrowMorningTemp < WorldClimate.SNOW_TEMPERATURE ||
