    ...
    public static void eatingFood(LivingEntityUseItemEvent.Finish event) {
        if (event.getEntityLiving() != null && !event.getEntityLiving().world.isRemote && event.getEntityLiving() instanceof ServerPlayerEntity) {
            ...
            if (adj != null) {
                ...
                if(heat>1) {
                	event.getEntityLiving().attackEntityFrom(FHDamageSources.hyperthermiaFrom(event.getEntityLiving()),(heat)*2);
                }else if(heat<-1) 
                	event.getEntityLiving().attackEntityFrom(FHDamageSources.hypothermiaFrom(event.getEntityLiving()),(heat)*2);
                if (heat > 0) {
