 ...
 */
public class ToggleDeleteOverflowMessage implements CMessage {
    public ToggleDeleteOverflowMessage(FriendlyByteBuf buffer) {
    }

    public ToggleDeleteOverflowMessage() {
    }
    ...
    public void handle(Supplier<NetworkEvent.Context> context) {
        if(context.get().getSender() != null){
            if(Objects.requireNonNull(context.get().getSender()).containerMenu instanceof SnowSackMenu snowSackMenu){
                snowSackMenu.toggleDeleteOverflow();
            }
        }
    }
}
