...
class HelpPanel extends Panel {
    ...
    public void addWidgets() {
        float scale = Float.parseFloat(GuiUtils.translateGui("minigame.scale").getString());
        RTextField t1 = new RTextField(this).setColor(TechIcons.text).addFlags(0).setScale(scale)
                .setMaxWidth(112).setMaxLine(2).setText(GuiUtils.translateGui("minigame.t1"));
        t1.setPos(8, 70);
        RTextField t2 = new RTextField(this).setColor(TechIcons.text).addFlags(0).setScale(scale)
                .setMaxWidth(114).setMaxLine(2).setText(GuiUtils.translateGui("minigame.t2"));
        t2.setPos(124, 70);
        RTextField t3 = new RTextField(this).setColor(TechIcons.text).addFlags(0).setScale(scale)
                .setMaxWidth(90).setMaxLine(3).setText(GuiUtils.translateGui("minigame.t3"));
        t3.setPos(5, 137);
        RTextField t4 = new RTextField(this).setColor(TechIcons.text).addFlags(0).setScale(scale)
                .setMaxWidth(68).setMaxLine(3).setText(GuiUtils.translateGui("minigame.t4"));
        t4.setPos(95, 137);
        RTextField t5 = new RTextField(this).setColor(TechIcons.text).addFlags(0).setScale(scale)
                .setMaxWidth(74).setMaxLine(3).setText(GuiUtils.translateGui("minigame.t5"));
        t5.setPos(165, 137);
        add(t1);
        add(t2);
        add(t3);
    ...
    public void draw(MatrixStack arg0, Theme arg1, int arg2, int arg3, int arg4, int arg5) {
        if (ot.showHelp) {
            ...
            arg1.drawString(arg0, GuiUtils.translateGui("minigame.match"), arg2 + 8, arg3 + 90, TechIcons.text, 0);
            arg1.drawString(arg0, GuiUtils.translateGui("minigame.display"), arg2 + 8, arg3 + 2, TechIcons.text, 0);
        }
    }
