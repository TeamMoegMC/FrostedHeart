...
public abstract class MixinClockworkBearingTileEntity extends KineticTileEntity {
	...
	public float calculateStressApplied() {
		float stress=1;
		if(hourHand!=null) {
			stress+= hourHand.getContraption().getBlocks().size()*0.25F+hourHand.getContraption().getActors().size()*6F;
		}
		if(minuteHand!=null) {
			stress+= minuteHand.getContraption().getBlocks().size()*0.25F+minuteHand.getContraption().getActors().size()*6F;
		}
