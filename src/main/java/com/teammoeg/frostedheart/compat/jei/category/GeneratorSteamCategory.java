...
public class GeneratorSteamCategory implements IRecipeCategory<GeneratorSteamRecipe> {
    ...
    public GeneratorSteamCategory(IGuiHelper guiHelper) {
        ...
        this.ICON = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK,new ItemStack(FHMultiblocks.generator_t2));
    ...
    @Override
    public void draw(GeneratorSteamRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics transform, double mouseX, double mouseY) {
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder iRecipeLayoutBuilder, GeneratorSteamRecipe generatorSteamRecipe, IFocusGroup iFocusGroup) {
        iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.INPUT, 26, 12).addIngredients(FluidIngredientType.INSTANCE, generatorSteamRecipe.input.getMatchingFluidStacks());
        iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.INPUT, 75, 7).addIngredients(Ingredient.merge(generatorSteamRecipe.getIngredients()));
        iRecipeLayoutBuilder.addSlot(RecipeIngredientRole.OUTPUT, 75, 46).addItemStack(generatorSteamRecipe.getResultItem(null));
    }

    @Override
    public RecipeType<GeneratorSteamRecipe> getRecipeType() {
        return JEICompat.GeneratorSteam;
    }
    ...
    @Override
    public List<Component> getTooltipStrings(GeneratorSteamRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
