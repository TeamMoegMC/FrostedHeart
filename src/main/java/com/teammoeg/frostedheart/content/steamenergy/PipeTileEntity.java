...
public abstract class PipeTileEntity extends IEBaseTileEntity implements ITickableTileEntity{
    ...
    public void tick() {
        ...
        for (Direction d : Direction.values())
            bs.with(FluidPipeBlock.RIM_PROPERTY_MAP.get(d), block.shouldDrawRim(this.getWorld(), pos, bs, d));
        bs.with(FluidPipeBlock.CASING, block.shouldDrawCasing(this.getWorld(), pos, bs));
        if(obs!=bs)
