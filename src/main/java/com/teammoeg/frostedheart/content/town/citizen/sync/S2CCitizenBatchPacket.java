 ...
 * plus 1 byte direction and 1 byte state — roughly 7–9 bytes per entry.
 * The client extrapolates "snapshot + direction × state speed"; error is
...
public final class S2CCitizenBatchPacket implements CMessage {
	...
	public record Entry(int id, int lx, int ly, int lz, byte dir, byte state) {
	}
	...
	public S2CCitizenBatchPacket(FriendlyByteBuf buf) {
		...
		for (int g = 0; g < groupCount; g++) {
			...
			for (int i = 0; i < count; i++)
				entries.add(new Entry(buf.readVarInt(), buf.readByte() & 0xFF, buf.readShort(), buf.readByte() & 0xFF,
