	...
	public boolean work(Town town, HuntingBaseState workData, WorkOrder workOrder) {
        ...
        if (town instanceof TeamTown teamTown) {//the town must be team town because it needs to get all camps in the town.
            ...
            if (!campsUnchecked.isEmpty()) {
                double baseRating = workData.getRating();
                for (TownWorkerData data : campsUnchecked) {
                    AbstractMap.SimpleEntry<TownWorkerData, Double> dataPair = new AbstractMap.SimpleEntry<>(data, workData.getRating() * baseRating * 2);//double: 考虑到与之距离过近的camp之后新计算的rating    *2:下面遍历的时候会遍历到它自己
            ...
            int residentsLeft = workData.maxResidents;
