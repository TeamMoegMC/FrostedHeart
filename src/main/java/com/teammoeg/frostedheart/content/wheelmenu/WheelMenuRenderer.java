...
public class WheelMenuRenderer {
    ...
    private static final String OPEN_ANIM_NAME = WheelMenuRenderer.class.getName() + "opening";
    private static final String CLOSE_ANIM_NAME = WheelMenuRenderer.class.getName() + "closing";
    ...
    public static void render(ForgeGui gui, GuiGraphics graphics, float partialTicks, int width,
                       int height) {
        ...
        if (mouseMoved) {
            ...
            mouseMoved = !MouseHelper.isMouseIn(mouseX, mouseY, cw-5, ch-5, 10, 10);
        ...
        if (hoveredSelection != null) {
            CGuiHelper.drawCenteredStrings(graphics, font, font.split(hoveredSelection.getMessage(), (int)(WHEEL_INNER_RADIUS * 2 - 16)), 0, -4, hoveredSelection.color, 10, true ,true);
        } else {
            Component c = Component.translatable("gui.frostedheart.wheel_menu.message", FHKeyMappings.key_openWheelMenu.get().getKey().getDisplayName().getString());
            var texts = font.split(c, (int)(WHEEL_INNER_RADIUS * 2 - 16));
            CGuiHelper.drawCenteredStrings(graphics, font, texts, 0, -texts.size()*5, ColorHelper.CYAN, 10, true ,true);
        }
        pose.popPose();
    }
    ...
    protected static void init() {
        ...
        try {
            ...
            addSelection(new Selection(Component.translatable("key.ftbquests.quests"), FTBQuestsItems.BOOK, s -> FTBQuestsClient.openGui()));
        } catch (ClassNotFoundException e) {
            addSelection(new Selection(Component.literal("Open Quests"), Items.BOOK.getDefaultInstance(), Selection.NO_ACTION));
        }
        addSelection(new Selection(Component.translatable("gui.frostedheart.wheel_menu.selection.debug"), FHItems.debug_item.get().getDefaultInstance(), ColorHelper.CYAN, s -> ClientUtils.getPlayer().isCreative(), s -> DebugScreen.openDebugScreen(), Selection.NO_ACTION));
        addSelection(new Selection(Component.translatable("gui.frostedheart.wheel_menu.selection.nutrition"), NutritionScreen.fat_icon, s -> FHNetwork.sendToServer(new C2SOpenNutritionScreenMessage())));
        addSelection(new Selection(Component.translatable("gui.frostedheart.wheel_menu.selection.clothing"), FHItems.gambeson.get().getDefaultInstance(), s -> FHNetwork.sendToServer(new C2SOpenClothesScreenMessage())));
        tick();
    }
    ...
    public boolean keyReleased(int pKeyCode, int pScanCode, int pModifiers) {
        if (!closing && pKeyCode == FHKeyMappings.key_openWheelMenu.get().getKey().getValue()) {
            if (hoveredSelection != null) {
    ...
    public static class Selection {
         ...
         */
        public Selection(Component message, Object icon, int color, Predicate<Selection> visibility, Action pressAction, Action hoverAction) {
