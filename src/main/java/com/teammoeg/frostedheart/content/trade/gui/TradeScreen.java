...
import com.teammoeg.chorda.client.cui.widgets.OverlayItemSlot;
import com.teammoeg.chorda.client.cui.widgets.TextField;
import com.teammoeg.chorda.client.cui.widgets.ToolTipElement;
import com.teammoeg.chorda.client.cui.widgets.TristateCheckBox;
...
public class TradeScreen extends MenuPrimaryLayer<TradeContainer> {
	...
	public TradeScreen(TradeContainer container) {
		super(container);
		...
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 9; j++) {
				...
				OverlayItemSlot fs = new OverlayItemSlot(this) {
					...
					@Override
					public void getTooltip(TooltipBuilder tooltip) {
						if (tab.isChecked() || container.relations.sum() > TradeConstants.RELATION_TO_TRADE) {
		...
		rels = new RelationSlot(this, () -> container.relationMinus - container.discountAmount);
		rels.setTooltip(c -> {
			int relation = container.relations.sum();
			if (relation < TradeConstants.RELATION_TO_TRADE && !container.order.isEmpty()) {
				c.accept(Lang.translateGui("trade.unwilling").withStyle(ChatFormatting.DARK_RED));
				return;
			}
			if (container.discountAmount != 0)
				c.accept(Lang.translateGui("trade.discount").append(Components
					.str(" -" + (int) Math.ceil(container.discountAmount / 10f)).withStyle(ChatFormatting.GREEN)));
			if (container.relationMinus > 0)
				c.accept(Lang.translateGui("trade.bad_relation").append(
					Components.str(" " + (int) Math.ceil(container.relationMinus / 10f)).withStyle(ChatFormatting.RED)));
		...
		bargain = new TristateCheckBox(this, TradeIcons.BARGAINN, TradeIcons.BARGAINO, TradeIcons.BARGAIND) {
			...
			public boolean onClicked(MouseButton arg0) {
				if (bargain.isEnabled()) {
					FHNetwork.INSTANCE.sendToServer(new BargainRequestPacket(container.order));
					return true;
				}
				return true;
			}
		...
		trade = new TristateCheckBox(this, TradeIcons.DEALN, TradeIcons.DEALO, TradeIcons.DEALD) {
			...
			public boolean onClicked(MouseButton arg0) {
				if (trade.isEnabled()) {
					FHNetwork.INSTANCE.sendToServer(new TradeCommitPacket(container.order));
					return true;
				}
				return true;
			}
