...
public abstract class GeneratorLogic<T extends GeneratorLogic<T, ?>, R extends GeneratorState>
	...
	public void tryRegist(IMultiblockContext<R> ctx) {
		ctx.getState().tryRegist(ctx.getLevel().getRawLevel(), CMultiblockHelper.getAbsoluteMaster(ctx), (short) CMultiblockHelper.getMultiblock(ctx).masterPosInMB().getY());
	}

	public void regist(IMultiblockContext<R> ctx) {
		ctx.getState().regist(ctx.getLevel().getRawLevel(), CMultiblockHelper.getAbsoluteMaster(ctx), (short) CMultiblockHelper.getMultiblock(ctx).masterPosInMB().getY());
	}
