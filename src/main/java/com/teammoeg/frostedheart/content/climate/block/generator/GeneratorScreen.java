...
public class GeneratorScreen<R extends GeneratorState, T extends GeneratorLogic<T, R>> extends IEContainerScreen<GeneratorContainer<R, T>> {
	...
	public void updateTooltip() {
		...
		costStr.clear();
		// Optional<IMultiblockBEHelper<?>> ohelper =
		// CMultiblockHelper.getBEHelperOptional(Minecraft.getInstance().level,
		// menu.pos.getValue());
	...
	protected List<InfoArea> makeInfoAreas() {
		if (menu.getTank() != null)
			fia = new FluidInfoArea(menu.getTank(), new Rect2i(leftPos + 135, topPos + 57, 16, 60), 0, 0, 0, 0, TEXTURE);
		return super.makeInfoAreas();
		// return ImmutableList.of(new FluidInfoArea(menu.getTank(), new
		// Rect2i(leftPos+135,topPos+ 57, 16, 60), 0, 0, 0, 0, TEXTURE));
	}
	...
	protected void drawContainerBackgroundPre(GuiGraphics matrixStack, float partialTicks, int x, int y) {
		// background
		// matrixStack.blit(TEXTURE, 0, 0, 0, 0, this.imageWidth,
		// this.imageHeight,TEXW,TEXH);
		...
		// generator symbol
		generatorSymbol.blitAtlas(matrixStack, leftPos, topPos, generatorPos, (tierOffset != 0 || menu.isBroken.getValue()) ? 0 : ((menu.isWorking.getValue() && menu.process.getValue() > 0) ? 2 : 1),
			(menu.getTier() - 1 + tierOffset));
	...
	protected void gatherAdditionalTooltips(int mouseX, int mouseY, Consumer<Component> addLine, Consumer<Component> addGray) {
		...
		if (fia!=null&&isMouseIn(mouseX, mouseY, 135, 57, 16, 60)) {
			addLine.accept(Lang.translateGui("generator.steam.level",Integer.toString(menu.getTank().getFluidAmount())));
			if(menu.getTank().getFluidAmount()>=menu.getTank().getCapacity()) {
				addLine.accept(Lang.translateGui("generator.steam.fulfilled"));
			}
		}
