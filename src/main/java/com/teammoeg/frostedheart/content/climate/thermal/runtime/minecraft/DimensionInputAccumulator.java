...
/** Main-thread-only coalescer for the next not-yet-submitted dimension batch. */
final class DimensionInputAccumulator {
    ...
    void changeSourcePower(long sourceId, double powerW, long effectiveTick) {
        sourceEvents.addPowerChange(sourceId, powerW, effectiveTick);
    }

    void changeSourceEnabled(long sourceId, boolean enabled, long effectiveTick) {
        sourceEvents.addEnabledChange(sourceId, enabled, effectiveTick);
    }

    void emitSourceImpulse(
            long sourceId,
            int portId,
    ...
    ThermalInputBatch seal(long targetTick) {
