...
final class AirRouteCompiler {
    ...
    private final Long2ObjectOpenHashMap<ObjectOpenHashSet<Component>> bySection = new Long2ObjectOpenHashMap<>();
    private final Long2ObjectOpenHashMap<ArrayList<EdgeSlice>> edgesByBrick = new Long2ObjectOpenHashMap<>();
    ...
    AirRouteCompiler(ThermalSignatureTable signatures, ThermalCellArena arena, int maximumPositions) {
        this.signatures = signatures;
        this.arena = arena;
        this.maximumPositions = maximumPositions;
    }

    boolean hasPendingWork() { return !pending.isEmpty() || !seeds.isEmpty() || !dirtyBricks.isEmpty(); }
    long lastVisitCount() { return lastVisitCount; }
    void beginCut() { lastVisitCount = 0; invalidatedSourceSections.clear(); }
    long[] invalidatedSourceSections() { return invalidatedSourceSections.toLongArray(); }
    ...
    AirRouteValidity validity(long position) { return atPosition.get(position).validity; }
    ...
    void invalidate(ThermalInputBatch batch) {
        ...
        for (var retirement : batch.retirements()) changedSections.add(retirement.page().sectionKey());
        for (var residency : batch.residencyUpdates()) changedSections.add(residency.page().sectionKey());
        for (int index = 0; index < batch.geometry().size(); index++) changedSections.add(batch.geometry().page(index).sectionKey());
    ...
    void prepare(TopologyView view, IntOpenHashSet fragments) {
        ...
        for (int fragment : initial) {
            ...
            for (int block = 0; block < 64; block++) {
                ...
                int signature = view.signatureAtWorld(BlockPos.getX(position), BlockPos.getY(position), BlockPos.getZ(position));
                if (signatures.materialProfileId(signature) == 0 || signatures.ventilation(signature) == 0
                        || atPosition.containsKey(position) || atPosition.size() >= maximumPositions) continue;
                seeds.add(position);
            }
        }
        ...
        while (budget >= 12 && (!pending.isEmpty() || !seeds.isEmpty())) {
            if (pending.isEmpty()) {
                ...
                budget--;
                int signature = view.signatureAtWorld(BlockPos.getX(position), BlockPos.getY(position), BlockPos.getZ(position));
                if (atPosition.containsKey(position) || atPosition.size() >= maximumPositions
                        || signatures.materialProfileId(signature) == 0 || signatures.ventilation(signature) == 0) continue;
    ...
    private void includeDirtyFragments(TopologyView view, IntOpenHashSet fragments) {
        for (long position : dirtyBricks) {
            ...
            if (page != null && view.resident(page, brick)) fragments.add(page.fragmentIndex(brick));
        }
    }
    ...
    boolean hasRegion(long position) {
        ...
        return component != null && component.ready && component.alive
                && Double.isFinite(component.distance[component.indexes.get(position)]);
    }

    long region(long position) {
        Component component = atPosition.get(position);
        return component.labels[component.indexes.get(position)];
    }

    boolean closed(long position) {
        Component component = atPosition.get(position);
        return component != null && component.alive && component.ready
                && component.portRegions.isEmpty() && component.missingBricks.isEmpty();
    }

    double normalizedResistance(long position) {
        Component component = atPosition.get(position);
        return component.distance[component.indexes.get(position)];
    }
    ...
    void appendEdges(long ownerBrick, TopologyView view, double mixing, ThermalFragment.RoutedContacts.Builder pairs) {
        ...
        for (EdgeSlice slice : slices) {
            ...
            for (int edge : slice.indexes) {
                ...
                if (first >= 0 && second >= 0 && first != second) {
                    pairs.add(first, second, mixing / component.edgeResistance[edge], component.validity);
                }
            }
        }
    }
    ...
    void collectRequiredBricks(WorkerPageStore pages, Long2LongOpenHashMap desired) {
        for (Component component : components) {
            ...
            for (long brick : component.memberBricks) {
                if (pages.hasThermalInterestAt(brick)) { interested = true; break; }
            }
    ...
    private static void requireBrick(Long2LongOpenHashMap desired, long position) {
        ...
        int brick = (BlockPos.getX(position) & 15) >>> 2 | ((BlockPos.getZ(position) & 15) >>> 2) << 2
    ...
    private final class Component {
        ...
        final LongOpenHashSet memberBricks = new LongOpenHashSet(), missingBricks = new LongOpenHashSet(), edgeOwners = new LongOpenHashSet();
        ...
        final Long2ObjectOpenHashMap<Long2DoubleOpenHashMap> candidates = new Long2ObjectOpenHashMap<>();
        boolean alive = true, ready;
        int stage, cursor, seedCursor;
        double[] distance;
        long[] labels, edgeFirst, edgeSecond;
        double[] edgeResistance;
        int[] heap, heapPositions;
        int heapSize;
        AirRouteValidity validity;

        Component() { indexes.defaultReturnValue(-1); }
        ...
        void add(long position, int signature, TopologyView view) {
            ...
            if (view.brickAtWorld(BlockPos.getX(position), BlockPos.getY(position), BlockPos.getZ(position)) != null) {
            ...
            while (alive && !ready && budget - used >= 12) {
                if (stage == 0) {
                    if (cursor == positions.size()) {
                        distance = new double[positions.size()];
                        Arrays.fill(distance, Double.POSITIVE_INFINITY);
                        labels = new long[positions.size()];
                    ...
                    for (int face = 0; face < 6; face++) {
                        long neighbor = BlockPos.asLong(x + TopologyView.DX[face], y + TopologyView.DY[face], z + TopologyView.DZ[face]);
                        dependency(neighbor, view);
                        int signature = view.signatureAtWorld(BlockPos.getX(neighbor), BlockPos.getY(neighbor), BlockPos.getZ(neighbor));
                        if (!signatures.valid(signature)) { missingBricks.add(brick(neighbor)); continue; }
                        if (signatures.isAir(signature)) {
                            int airSlot = view.airSlotAt(neighbor);
                            if (airSlot < 0) { missingBricks.add(brick(neighbor)); continue; }
                            long region = view.airRegionAt(neighbor);
                            int axis = face / 2;
                            double plane = (axis == 0 ? x : axis == 1 ? y : z) + (face % 2 == 0 ? 0 : 1);
                            double resistance = 50.0 / ventilation.getInt(node) + Math.max(0.5, Math.abs(arena.center(airSlot, axis) - plane));
                            portNodes.add(node); portRegions.add(region); portResistance.add(resistance);
                            memberBricks.add(brick(region));
                        } else if (signatures.ventilation(signature) > 0 && indexes.get(neighbor) < 0) {
                            Component previous = atPosition.get(neighbor);
                            if (previous != null && previous != this) discard(previous);
                            if (atPosition.size() >= maximumPositions) { missingBricks.add(brick(neighbor)); continue; }
                            add(neighbor, signature, view);
                        }
                    }
                    used += 6;
                } else if (stage == 1) {
                    if (seedCursor == portNodes.size()) { stage = 2; continue; }
                    int port = seedCursor++;
                    improve(portNodes.getInt(port), portResistance.getDouble(port), portRegions.getLong(port));
                    used++;
                } else if (stage == 2) {
                    if (heapSize == 0) { stage = 3; cursor = 0; seedCursor = 0; continue; }
                    int node = pop();
                    long position = positions.getLong(node);
                    for (int face = 0; face < 6; face++) {
                        long neighbor = adjacent(position, face);
                        int next = indexes.get(neighbor);
                        if (next >= 0) improve(next, distance[node] + 50.0 / ventilation.getInt(node)
                                + 50.0 / ventilation.getInt(next), labels[node]);
                    }
                    used += 6;
                } else {
                    if (cursor < positions.size()) {
                        int node = cursor++;
                        if (Double.isFinite(distance[node])) for (int face = 1; face < 6; face += 2) {
                            int next = indexes.get(adjacent(positions.getLong(node), face));
                            if (next >= 0 && Double.isFinite(distance[next])) candidate(labels[node], labels[next], distance[node] + distance[next]
                                    + 50.0 / ventilation.getInt(node) + 50.0 / ventilation.getInt(next));
                        }
                        used += 6;
                    } else if (seedCursor < portNodes.size()) {
                        int port = seedCursor++, node = portNodes.getInt(port);
                        candidate(labels[node], portRegions.getLong(port), distance[node] + portResistance.getDouble(port));
                        used++;
                    } else finish(view);
                }
            }
        ...
        void candidate(long first, long second, double resistance) {
            ...
            var targets = candidates.computeIfAbsent(low, ignored -> {
                var result = new Long2DoubleOpenHashMap(); result.defaultReturnValue(Double.POSITIVE_INFINITY); return result;
            });
        ...
        void finish(TopologyView view) {
            ...
            for (var dependency : dependencies.long2LongEntrySet()) {
                ...
                if (view.geometryRevision(page) != dependency.getLongValue()) { discard(this); return; }
                handles.add(page.handle); revisions.add(dependency.getLongValue());
            }
            validity = new AirRouteValidity(handles.toArray(ThermalPageHandle[]::new), revisions.toLongArray());
            int count = 0;
            for (var values : candidates.values()) count += values.size();
            edgeFirst = new long[count]; edgeSecond = new long[count]; edgeResistance = new double[count];
            var ownerIndexes = new Long2ObjectOpenHashMap<IntArrayList>();
            long[] sources = candidates.keySet().toLongArray(); Arrays.sort(sources);
            int edge = 0;
            for (long first : sources) {
                var targets = candidates.get(first);
                long[] seconds = targets.keySet().toLongArray(); Arrays.sort(seconds);
                for (long second : seconds) {
                    edgeFirst[edge] = first; edgeSecond[edge] = second; edgeResistance[edge] = targets.get(second);
                    ownerIndexes.computeIfAbsent(brick(first), ignored -> new IntArrayList()).add(edge++);
                }
            }
            for (var owner : ownerIndexes.long2ObjectEntrySet()) {
                edgesByBrick.computeIfAbsent(owner.getLongKey(), ignored -> new ArrayList<>()).add(new EdgeSlice(this, owner.getValue()));
        ...
        void improve(int node, double nextDistance, long label) {
            if (nextDistance > distance[node] || nextDistance == distance[node] && label >= labels[node]) return;
            distance[node] = nextDistance; labels[node] = label;
            int index = heapPositions[node];
            if (index < 0) { index = heapSize++; heap[index] = node; heapPositions[node] = index; }
            while (index > 0) {
        ...
        boolean less(int first, int second) {
            int order = Double.compare(distance[first], distance[second]);
            if (order != 0) return order < 0;
            order = Long.compare(labels[first], labels[second]);
        ...
        void swap(int first, int second) {
            ...
            heapPositions[heap[first]] = first; heapPositions[heap[second]] = second;
        }
    }
    ...
    static long brickPosition(WorkerPageStore.PageState page, int brick) {
        return BlockPos.asLong(SectionPos.sectionToBlockCoord(SectionPos.x(page.handle.sectionKey())) + (brick & 3) * 4,
                SectionPos.sectionToBlockCoord(SectionPos.y(page.handle.sectionKey())) + (brick >>> 4 & 3) * 4,
                SectionPos.sectionToBlockCoord(SectionPos.z(page.handle.sectionKey())) + (brick >>> 2 & 3) * 4);
    }
    static long blockPosition(long origin, int block) {
        return BlockPos.asLong(BlockPos.getX(origin) + (block & 3), BlockPos.getY(origin) + (block >>> 4), BlockPos.getZ(origin) + (block >>> 2 & 3));
    }
    private static long adjacent(long position, int face) {
        return BlockPos.asLong(BlockPos.getX(position) + TopologyView.DX[face], BlockPos.getY(position) + TopologyView.DY[face], BlockPos.getZ(position) + TopologyView.DZ[face]);
    }
    private static long brick(long position) { return BlockPos.asLong(BlockPos.getX(position) & ~3, BlockPos.getY(position) & ~3, BlockPos.getZ(position) & ~3); }
    private static long section(long position) { return SectionPos.asLong(BlockPos.getX(position) >> 4, BlockPos.getY(position) >> 4, BlockPos.getZ(position) >> 4); }
}
