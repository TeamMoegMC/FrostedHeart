		...
		LiteralArgumentBuilder<CommandSource> run=Commands.literal("scenario").then(
			...
			Commands.argument("scenario", StringArgumentType.string()).then(
				Commands.argument("label", StringArgumentType.string()).executes(ct->{
					FHScenario.get(ct.getSource().asPlayer()).jump(
						new ExecuteTarget(FHScenario.get(ct.getSource().asPlayer()),StringArgumentType.getString(ct, "scenario"),StringArgumentType.getString(ct, "label")));
					return Command.SINGLE_SUCCESS;
				})
				).executes(ct->{
					
					FHScenario.get(ct.getSource().asPlayer()).jump(
						new ExecuteTarget(FHScenario.get(ct.getSource().asPlayer()),StringArgumentType.getString(ct, "scenario"),null));
					return Command.SINGLE_SUCCESS;
				})
			)).then(Commands.literal("queue").then(
				Commands.argument("scenario", StringArgumentType.string()).then(
					Commands.argument("label", StringArgumentType.string()).executes(ct->{
						FHScenario.get(ct.getSource().asPlayer()).queue(
							new ExecuteTarget(FHScenario.get(ct.getSource().asPlayer()),StringArgumentType.getString(ct, "scenario"),StringArgumentType.getString(ct, "label")));
						return Command.SINGLE_SUCCESS;
					})
					).executes(ct->{
