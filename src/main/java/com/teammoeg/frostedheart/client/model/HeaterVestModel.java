...
public class HeaterVestModel<T extends LivingEntity> extends FHArmorBaseModel<T> {
    ...
    public void render(MatrixStack matrixStackIn, IVertexBuilder bufferIn, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
        if (entityTemp instanceof LivingEntity) {
            ...
            float storage = 0;
            if (!chest.isEmpty() && chest.getItem() instanceof HeaterVestItem)
                powerpack = chest;
            else if (!chest.isEmpty() && chest.getItem() instanceof ArmorItem && ItemNBTHelper.hasKey(chest, FHNBT.NBT_HEATER_VEST))
                powerpack = ItemNBTHelper.getItemStack(chest, FHNBT.NBT_HEATER_VEST);
            else if (FHBipedLayerRenderer.HEATER_VEST_PLAYERS.containsKey(entityTemp.getUniqueID()))
                powerpack = FHBipedLayerRenderer.HEATER_VEST_PLAYERS.get(entityTemp.getUniqueID()).getLeft();
    ...
    public static HeaterVestModel getModel() {
        if (modelInstance == null)
            modelInstance = new HeaterVestModel(.0625f, 0, 64, 32);
