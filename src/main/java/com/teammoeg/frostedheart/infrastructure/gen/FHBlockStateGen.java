...
public class FHBlockStateGen {
	...
	public static <T extends Block> NonNullBiConsumer<DataGenContext<Block, T>, RegistrateBlockstateProvider> farmland() {
		return (c, p) -> {
			...
			/*
			 * for(int j = 0; j < 7; j++) { p.getVariantBuilder(layer)
			 * .partialState().with(FertilizedFarmlandBlock.FERTILIZER,
			 * 0).with(FertilizedFarmlandBlock.MOISTURE,j)
			 * .modelForState().modelFile(origin).addModel(); } p.getVariantBuilder(layer)
			 * .partialState().with(FertilizedFarmlandBlock.FERTILIZER,
			 * 0).with(FertilizedFarmlandBlock.MOISTURE,7)
	...
	public static <T extends Block> NonNullBiConsumer<DataGenContext<Block, T>, RegistrateBlockstateProvider> horizontalFrontBlock(
		String modelName,
		String frontTexture,
		String sideTexture) {
		return (c, p) -> {
			...
			BlockModelBuilder modelBuilder = p.models()
				.withExistingParent(modelName, p.mcLoc("block/orientable"))
