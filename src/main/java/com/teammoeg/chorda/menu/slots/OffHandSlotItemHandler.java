...
/**
 * 基于 {@link IItemHandler} 的副手装备槽，继承 {@link ArmorSlotItemHandler}，用于在使用Forge物品处理器的自定义菜单中管理副手物品。
 * <p>
 * Off-hand equipment slot based on {@link IItemHandler}, extending {@link ArmorSlotItemHandler}, used to manage
 * off-hand items in custom menus that use Forge item handlers. Unlike armor slots, this slot accepts any item
 * and uses the shield icon as the empty slot indicator.
 *
 * @see OffHandSlot
 */
public class OffHandSlotItemHandler extends ArmorSlotItemHandler {
	 ...
	 * 构造一个基于物品处理器的副手槽位。
	 * <p>
	 * Constructs an off-hand slot backed by an item handler.
	 *
