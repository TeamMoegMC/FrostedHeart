...
public class CGuiHelper {
	...
	public static void drawItem(GuiGraphics guiGraphics, ItemStack stack, int x, int y, int zindex, float scaleX,
		...
		if (!stack.isEmpty()) {
			...
			if (!flag) {
				matrix4f = new Matrix4f(guiGraphics.pose().last().pose()).rotationYXZ(1.0821041F, 3.2375858F, 0.0F)
	...
	public static void drawItemWithReadableCount(GuiGraphics guiGraphics, ItemStack stack, int x, int y, int zindex,
			boolean drawDecorations, int count) {
		guiGraphics.pose().pushPose();
		guiGraphics.pose().translate(x, y, zindex + 150);
		if (!stack.isEmpty()) {
			BakedModel bakedmodel = ClientUtils.getMc().getItemRenderer().getModel(stack, ClientUtils.getMc().level,
		...
		if (drawDecorations) {
			String countReadable = CFormatHelper.toReadableItemStackUnit(count);
			Font font = ClientUtils.getMc().font;
			boolean needResize = count > 999;
			int width = font.width(countReadable);
			if (needResize) {
				guiGraphics.pose().translate(x + 19 - width, y + 11, 350.0F);
				guiGraphics.pose().scale(0.8f, 0.8f, 1f);

			}
			renderItemDecorationsWithoutCount(guiGraphics, font, stack, 0, 0);
		}
		guiGraphics.pose().popPose();
	}
	...
	public static void renderItemDecorationsWithoutCount(GuiGraphics guiGraphics, Font pFont, ItemStack pStack, int pX,
		...
		if (!pStack.isEmpty()) {
			...
			float f = localplayer == null ? 0.0F
					: localplayer.getCooldowns().getCooldownPercent(pStack.getItem(), mc.getFrameTime());
			if (f > 0.0F) {
			...
			guiGraphics.pose().popPose();
			net.minecraftforge.client.ItemDecoratorHandler.of(pStack).render(guiGraphics, pFont, pStack, pX, pY);
		}
	}
	 ...
	 * 渲染列表中的所有文本
	 *
	 * @param texts      支持 {@link FormattedCharSequence} {@link Component}
	 *                   {@link Object}
	 * @param lineSpace  行间距
	 ...
	 *                   <p>
	 *                   {@link Alignment#CENTER} 以给予的坐标为中心绘制居中对齐文本
	 *                   <p>
	 *                   {@link Alignment#RIGHT} 在给予的坐标左侧绘制右对齐文本
	 *                   <p>
	 */
	public static void drawStringLines(GuiGraphics graphics, Font font, List<?> texts, int x, int y, int color,
	 ...
	 *                   <p>
	 *                   {@link Alignment#CENTER} 在区域中心绘制居中对齐文本
	 *                   <p>
	 *                   {@link Alignment#RIGHT} 在区域右侧绘制右对齐文本
	 *                   <p>
	 */
	 ...
	 *                        <p>
	 */
	public static void drawStringLinesInBound(GuiGraphics graphics, Font font, List<?> texts, int x, int y,
			int maxWidth, int color, int lineSpace, boolean shadow, int backgroundColor, Alignment alignment) {
		if (texts.isEmpty())
			return;
	...
	public static int getFluidColor(FluidStack fluid) {
		...
		return FastColor.ARGB32.color(FastColor.ABGR32.alpha(abgr), FastColor.ABGR32.red(abgr),
