...
public class CookieCommand {
	...
	public void sendCookie(ScenarioCommandContext runner) {
		...
		if(obj instanceof CompoundTag tag) {
			FSNetwork.INSTANCE.sendPlayer((ServerPlayer) runner.context().player(), new S2CSetCookiesMessage(tag));
	...
	public void requestCookie(ScenarioCommandContext runner) {
		...
		if(obj instanceof ArrayList tag) {
			FSNetwork.INSTANCE.sendPlayer((ServerPlayer) runner.context().player(), new S2CRequestCookieMessage(runner.thread().getRunId(),tag));
