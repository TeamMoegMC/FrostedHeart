...
package com.teammoeg.frostedscenario.commands.client;
...
import com.teammoeg.frostedscenario.client.IClientScene;
import com.teammoeg.frostedscenario.client.dialog.HUDDialog;
import com.teammoeg.frostedscenario.client.dialog.ImageScreenDialog;
import com.teammoeg.frostedscenario.client.gui.layered.LayerManager;
import com.teammoeg.frostedscenario.client.gui.layered.Transition;
import com.teammoeg.frostedscenario.client.gui.layered.gl.GLTextContent;
import com.teammoeg.frostedscenario.client.gui.layered.java2d.GraphicsImageContent;
import com.teammoeg.frostedscenario.client.gui.layered.java2d.GraphicsLineContent;
import com.teammoeg.frostedscenario.client.gui.layered.java2d.GraphicsRectContent;
import com.teammoeg.frostedscenario.client.gui.layered.java2d.GraphicsTextContent;
...
@ScenarioCommandProvider(clientOnly=true)
public class ClientControl implements IClientControlCommand {
		...
		if(tsk instanceof ItemTask itmtask) {
			...
			if(ItemFiltersAPI.isFilter(stack)) {
				...
				Lazy<CompoundTag> tagBuilder=Lazy.of(()->{
					...
					b.addString(Component.Serializer.toJson(new LangBuilder("frostedheart").translate("tooltip","suitable_items").component()));
					
					for(ItemStack is:toshow) {
						b.addString(Component.Serializer.toJson(new LangBuilder("frostedheart").text("- ").add(is.getDisplayName()).component()));
					}
					return disp.build();
				});
			...
			itt= new LangBuilder("frostedheart").translate("message","item_task",cmp).component();
			
		}else if(tsk instanceof KillTask) {
			itt= new LangBuilder("frostedheart").translate("message","kill_task",tsk.getTitle().copy().withStyle(Style.EMPTY.withColor(ChatFormatting.YELLOW))).component();
		}else {
			itt= new LangBuilder("frostedheart").translate("message","other_task",tsk.getTitle().copy().withStyle(Style.EMPTY.withColor(ChatFormatting.YELLOW))).component();
		}
		runner.cls();
	...
	public void bgm(IClientScene runner,@Param("n")@Param("name")String name) {
		...
		sound.ifPresentOrElse(t->ClientUtils.getMc().getMusicManager().startPlaying(new Music(t, 0, 0, true)), ()->{
			FSMain.LOGGER.error("Music "+name+" Not found");
		});
