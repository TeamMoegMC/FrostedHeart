...
public class ScenarioExecutor<T> {
    ...
    public void register(Class<?> clazz) {
        try {
            ...
            FSMain.LOGGER.error(MARKER, "Error registering scenario class",e);
        } catch (InvocationTargetException e) {
    ...
    public void registerCommand(String cmdName, ScenarioMethod<T> method) {
    	...
    	FSMain.LOGGER.debug(MARKER,"added command {}",cmdName.toLowerCase());
    }

    public void registerInst(Object clazz) {
        for (Method met : clazz.getClass().getDeclaredMethods()) {
            if (Modifier.isPublic(met.getModifiers())) {
                try {
                    ...
                    ex.printStackTrace();
                    FSMain.LOGGER.warn(MARKER, ex.getMessage());
                }
            }
        }
    }
