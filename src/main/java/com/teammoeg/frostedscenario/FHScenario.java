...
import com.teammoeg.frostedscenario.parser.Scenario;
import com.teammoeg.frostedscenario.parser.ScenarioParser;
import com.teammoeg.frostedscenario.parser.providers.ScenarioProvider;
...
import com.teammoeg.frostedscenario.runner.ScenarioContext;
import com.teammoeg.frostedscenario.runner.trigger.IVarTrigger;
...
public class FHScenario {
	...
	public static void startFor(ServerPlayer pe,String lang) {
		...
		FSMain.LOGGER.debug(MARKER, "From saved: "+pe.getLanguage()+" From packet: "+lang);
		sr.init(pe,lang);
	...
	public static Scenario loadScenario(ScenarioContext ctx,String name) {
		...
		try {
			for (ScenarioProvider i : scenarioProviders) {
				try {
					...
					if (s != null) {
						FSMain.LOGGER.info("Loaded scenario from provider "+i.getName());
						return s;
					}
				}catch(Exception e) {
			...
			if(f.exists()) {
				FSMain.LOGGER.info("Loading scenario from "+f.getAbsolutePath());
		...
		FSMain.LOGGER.error("Scenario "+name+" not found, falling back to empty scenario");
	...
	public static void callClientCommand(String name, ScenarioCommandContext runner, Map<String, String> params) {
		FSNetwork.INSTANCE.sendPlayer((ServerPlayer) runner.context().player(),
	...
	public static void registerClientDelegate(Class<?> cls) {
		for (Method met : cls.getDeclaredMethods()) {
			if (Modifier.isPublic(met.getModifiers())&&met.getParameterCount()>0&&met.getParameterTypes()[0].isAssignableFrom(IClientScene.class)) {
