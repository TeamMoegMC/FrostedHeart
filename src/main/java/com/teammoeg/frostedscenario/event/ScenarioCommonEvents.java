...
import com.teammoeg.frostedscenario.EventTriggerType;
import com.teammoeg.frostedscenario.FHScenario;
import com.teammoeg.frostedscenario.FSCapabilities;
...
@Mod.EventBusSubscriber(modid = FSMain.MODID, bus = Mod.EventBusSubscriber.Bus.FORGE)
public class ScenarioCommonEvents {
    ...
    public static void attachToPlayer(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof ServerPlayer) {//server-side only capabilities
            ...
            if (!(player instanceof FakePlayer)) {
            	//for compatibility use frostedheart namespace
                event.addCapability(new ResourceLocation("frostedheart", "scenario" ), FSCapabilities.SCENARIO.provider());
            }
        }
    }
	@SubscribeEvent
	public static void death(PlayerEvent.Clone ev) {
		...
		CUtils.clonePlayerCapability(FSCapabilities.SCENARIO, ev.getOriginal(), ev.getEntity());
		ev.getOriginal().invalidateCaps();
	}
    @SubscribeEvent
    public static void tickPlayer(TickEvent.PlayerTickEvent event) {
        if (FSConfig.SERVER.enableScenario.get() && event.side == LogicalSide.SERVER && event.phase == TickEvent.Phase.END && event.player instanceof ServerPlayer player) {
            // Scenario runner
    ...
    public static void doPlayerInteract(PlayerInteractEvent ite) {
        if(FSConfig.SERVER.enableScenario.get() && ite.getEntity() instanceof ServerPlayer&&!(ite.getEntity() instanceof FakePlayer)) {
