...
package com.teammoeg.frostedscenario.client.gui.layered.font;
...
import com.teammoeg.frostedscenario.client.gui.layered.font.UnihexParser.OverrideRange;
...
public class KGlyphProvider extends SimplePreparableReloadListener<Object> {
	...
	public void readFont(JsonObject jo) {
		FSMain.LOGGER.info("loading fonts...");
		JsonArray ja = jo.get("providers").getAsJsonArray();
		for (int i = 0; i < ja.size(); i++) {

			JsonObject cr = ja.get(i).getAsJsonObject();
			FSMain.LOGGER.info("loading provider " + cr.get("type").getAsString());
						...
						if (resource.isPresent())
							try (InputStream streamImg = resource.get().open()) {
								...
								if (image != null) {
									...
									FSMain.LOGGER.info("Error loading " + rrl);
								}
							}
					}
				}
				...
				try (InputStream stream = rm.open(new ResourceLocation(file))) {
					try (ZipInputStream zipinputstream = new ZipInputStream(stream)) {
						...
						while ((zipentry = zipinputstream.getNextEntry()) != null) {
							...
							FSMain.LOGGER.info("Got " + s+" from zipped file");
							if (s.endsWith(".hex")) {
