...
import com.teammoeg.frostedscenario.client.gui.layered.gl.TypedDynamicTexture;
...
public class PrerenderParams {
	Graphics2D g2d;
	BufferedImage image;
	int width=1024*FSConfig.CLIENT.getScenarioScale();
	int height=576*FSConfig.CLIENT.getScenarioScale();
	double scale;
	public PrerenderParams() {
		image=new BufferedImage(width,height, BufferedImage.TYPE_INT_ARGB);
		g2d=image.createGraphics();
		if(FSConfig.CLIENT.scenarioAntiAliasing.get()) {
	...
	public int calculateScaledX(int x) {
		...
		return (int) (x*scale);*/
		return x*FSConfig.CLIENT.getScenarioScale()/2;
	}
	public int calculateScaledY(int y) {
		...
		return (int) (y*scale);*/
		return y*FSConfig.CLIENT.getScenarioScale()/2;
	}
	public Rect calculateRect(int x,int y,int w,int h) {
		int nx=x*FSConfig.CLIENT.getScenarioScale()/2;
		int ny=y*FSConfig.CLIENT.getScenarioScale()/2;
		int nw=w*FSConfig.CLIENT.getScenarioScale()/2;
		int nh=h*FSConfig.CLIENT.getScenarioScale()/2;
		return new Rect(nx,ny,nw,nh);
	}
	public int calculateScaledSize(int s) {
		//return (int) (s*scale);
		return s*FSConfig.CLIENT.getScenarioScale()/2;
	}
