...
import com.teammoeg.frostedscenario.client.gui.layered.PrerenderParams;
import com.teammoeg.frostedscenario.client.gui.layered.RenderableContent;
			...
			try {
				...
				if (resource.isPresent()) {
					try (
						...
						FSMain.LOGGER.fatal(showingImage + " load error",e);
					}
				} else {
					this.cachedImage=null;
					FSMain.LOGGER.error(showingImage + " not found");
				}
			}catch(Throwable t) {
				this.cachedImage=null;
				FSMain.LOGGER.error("Exception when loading scenario image "+showingImage,t);
			}
		}, threadPool);
	}
