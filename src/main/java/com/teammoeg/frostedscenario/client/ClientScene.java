...
import com.teammoeg.frostedscenario.FSNetwork;
import com.teammoeg.frostedscenario.client.dialog.IScenarioDialog;
import com.teammoeg.frostedscenario.client.dialog.TextInfo;
import com.teammoeg.frostedscenario.client.dialog.TextInfo.SizedReorderingProcessor;
import com.teammoeg.frostedscenario.client.gui.layered.LayerManager;
import com.teammoeg.frostedscenario.mixin.NewChatGuiAccessor;
...
public class ClientScene implements IClientScene {
	private static GuiMessageTag SCENARIO=new GuiMessageTag(13684944, (GuiMessageTag.Icon)null, Components.translatable("chat.tag.frostedheart.scenario"), "Scenario");
	...
	public void setSpeed(double value) {
		value*=FSConfig.CLIENT.textSpeed.get();
		if(value<=2) {
	...
	public void sendContinuePacket(boolean isSkip) {
		// if(canSkip)
		FSNetwork.INSTANCE.sendToServer(new C2SScenarioResponsePacket(curTextId,isSkip, 0));
	...
	public boolean isTick() {
		...
		if (!unFinished && status==RunStatus.WAITCLIENT && ticksToContinue<=0) {
			if(sendImmediately) {
				...
				ticksToContinue = FSConfig.CLIENT.autoModeInterval.get();
			}
			hasText = false;
			canSkip = true;
	...
	public void sendClientReady() {
		FSNetwork.INSTANCE.sendToServer(new C2SClientReadyPacket(ClientUtils.getMc().getLanguageManager().getSelected()));
	}
	public void sendClientUpdate() {
		FSNetwork.INSTANCE.sendToServer(new C2SSettingsPacket());
	}
	@Override
