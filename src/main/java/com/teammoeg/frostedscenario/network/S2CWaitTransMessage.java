...
public record S2CWaitTransMessage(int thread,boolean isWaitTrans) implements CMessage{
	...
	public void handle(Supplier<Context> context) {
		context.get().enqueueWork(()->{
			if(isWaitTrans) {
				ClientScene.INSTANCE.onTransitionComplete.addListener(()->()->FSNetwork.INSTANCE.sendToServer(new C2SRenderingStatusMessage(thread,true)));
			}else {
				ClientScene.INSTANCE.onRenderComplete.addListener(()->()->FSNetwork.INSTANCE.sendToServer(new C2SRenderingStatusMessage(thread,false)));
			}
		});
