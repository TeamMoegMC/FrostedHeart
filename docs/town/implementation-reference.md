...
P2P bindings use a separate authority path: `TeamTown#bindOrRebindP2PTerminals`, `#setP2PTransportRate`, `#setP2PEndpointRedstonePowered`, and `#unbindP2PConnection`. `P2PBindingState` owns stable directed connections and indexes; the block entity never writes the reservation map or derives authoritative distance. A candidate rebind computes every added and removed reservation before committing binding state and `TownTransportState` together. Load recovery re-derives direct Manhattan distance from both stored `GlobalPos` facts. The warehouse registration and metric-refresh paths continue to reject `P2P_DIRECT_LINK`. See [p2p-logistics.md](p2p-logistics.md) for device behavior, formulas, redstone, transfer, and lifecycle guarantees.
