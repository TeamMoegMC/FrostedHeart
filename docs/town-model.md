...
> 状态：阶段 0–4 已实现；阶段 5 及之后尚未开始。2026-08-12 已把 T1、气候、居民、住宅、采矿和狩猎的 FH 自有参数统一到单一默认值来源，并加入凸缺粮损失与 `[0,40]°C` 外的独立温度压力。阶段 4 直接复用游戏的普通长期气候事件、方块温度、T1 半径/温升和整数球体判定，把建筑内部体素平均温度送入阶段 3 多日闭环；T2、加热器、热惯性与 T1 随机爬升仍不进入模拟。
>
...
`Scripts/town_scenarios/experiments/stage4-t1-population-sweep.json` 在同一 Java 模型内扫描 `1–200 resident` 的 `100` 个均匀整数点。所有人口点复用同一组 run seed，因此某一人口相对另一人口的差异是在相同气候样本下的配对比较。为避免把旧 8/24/48 参考场景中每人 `16 floor block` 的宽裕空间机械放大，扫描对每个人口分别构造满足**当前代码容量公式**的紧凑布局：
...
正式基准为每人口 `120 day × 1,000 fixed seed`；二项概率同时报告 Wilson 95% 区间。曲线的 100 点之外，额外模拟 `1/8/11/12/13/14/16/24/48/200` 十组储备轨迹，其中加粗 `13` 人曲线。关键结果如下：
...
扫描输出为 `population.csv`、`reserve-trajectories.csv` 和包含全部定义/参数/来源的 `summary.json`。`Scripts/plot_town_stage4.py` 读取该目录并按 `Figure_Guidelines.md` 生成五张无冗余标题的图：空间与温度可工作时段、燃料/食物自给率理论—模拟对照、四类结果概率、十组燃料/食物储备轨迹、覆盖率—气候下限解析图。图例不再使用含糊的 `house-coupled` / `hunting-coupled`，而直接写出温度判据。
