...
> 状态：阶段 0–3 已实现；阶段 4 及之后尚未开始。2026-08-12 已把 T1、居民、住宅、采矿和狩猎的 FH 自有参数统一到单一默认值来源，并加入凸缺粮损失与 `[0,40]°C` 外的独立温度压力。阶段 3 在恒定 `24°C` 下推进多日库存、居民状态、年龄/熟练度、粘性岗位、真实建筑顺序与有限 T1 燃料；气候与 T2 仍不进入模拟。
>
