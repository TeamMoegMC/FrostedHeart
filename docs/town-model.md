...
这里的 `meat` 只包括牛肉、猪肉、鸡肉、羊肉和兔肉；皮革、骨头、羽毛和兔皮不是食物。
...
生肉和熟肉当前都属于城镇一级居民食物，且没有额外 item-resource amount 配方，所以每个物品都等于 `1` 个食物资源单位。烟熏不会提高食物单位，只提高营养质量。
...
\[
SWE_{hunt|required\ per\ resident}=
\frac{c_{food}}{Y_{meat}}=
\frac{6.5}{1.1973684}=5.4286
\]

这意味着按当前值，一名居民即使贡献 `1 SWE` 狩猎也远不能养活自己；即使达到当前狩猎上限 `2.5 SWE`，期望肉产量也只有约 `2.9934 meat/day`，仍低于 `6.5`。这是当前系统无法自给的核心证据，不应被复杂机器或库存逻辑掩盖。
...
住宅可结算温度范围当前为 `[0,50]°C`。范围之外不是降低评分，而是整栋住宅不执行 `work`。
...
熟肉相对生肉主要提高 \(Q_N\) 和恢复速度，不改变 \(S_F\) 的分母或食物单位产量。
...
当前漏洞必须保留在第一版源码模型中：如果住宅温度低于 `0°C` 或高于 `50°C`，住宅不执行工作；已有居民仍保留住宅关联，因此当天不消耗食物，也不更新健康或精神。这不是保温或休眠模型，而是“冷住宅免费静止”。模拟器必须统计它，不能把它自动修成连续惩罚曲线。
...
- 维持一个居民食物所需劳动：`5.4286 hunting SWE/resident`
...
- 输出 SWE、煤产率、肉产率、居民食物需求和 T1 每日燃料消耗。
- 不运行多日模拟。
...
验收：确定性住宅公式精确；狩猎长期均值满足 `3 × 标准误`；明确复现冷住宅免费静止。
...
- 扫描四个核心轴：煤/SWE、肉/SWE、食物/居民日、塔燃料/激活日。
- 再扫描热场半径、温升和住宅阈值。
...
在任何完整模拟之前，当前默认值已经给出两条决定性约束：

1. T1 使用焦煤且无研究时，需要约 `9.1837 mining SWE` 才能仅维持塔的连续普通运行；8 名标准居民即使全部采矿仍不足。
2. 每名居民需要约 `5.4286 hunting SWE` 才能满足 `6.5` 食物单位；当前单人狩猎上限 `2.5 SWE` 仍无法养活该居民自己。

所以当前城镇闭环不可行不是气候随机性、机器吞吐或布局浪费首先造成的，而是煤、肉、人口需求和塔耗四个核心系数在量级上已经不闭合。后续模拟的第一职责是验证这些代数预测在库存、健康、气候和随机掉落加入后如何表现；第二职责才是寻找合理的新参数范围。
