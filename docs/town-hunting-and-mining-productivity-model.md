...
对居民 \(i\)，使用健康、精神、力量和智力四项范围为 0～100 的属性。狩猎和采矿各自拥有独立、非负的四项权重；代码计算前还会把属性限制在 0～100。

### 2.2 成年属性与既往职业经验初始化

新居民的健康和精神初始值均为 50。力量和智力各自独立生成一次：

$$
x=25(U_1+U_2+U_3+U_4),\qquad U_k\sim U(0,1)
$$

其范围为 0～100，期望为 50，标准差约为 14.4。力量和智力在成年居民加入城镇时确定并持久化；当前住宅、食物和工作结算均不会改变这两项属性。

狩猎与采矿分别保存独立的初始职业经验：

$$
p_0=50U^2,\qquad U\sim U(0,1)
$$

初始熟练度严格不超过 50，期望约为 16.7，中位数为 12.5。新居民创建时同时生成两项职业经验；旧居民缺失某个职业记录时只补充生成一次。
...
A_{i,j}=
\frac{
w_{j,h}x_{i,h}+w_{j,m}x_{i,m}+w_{j,s}x_{i,s}+w_{j,n}x_{i,n}
}{
w_{j,h}+w_{j,m}+w_{j,s}+w_{j,n}
}
$$
...
设 \(P_0\) 和 \(P_{100}\) 分别为加权属性 0 和 100、熟练度 0 时的生产力：

$$
S_{i,\mathrm{attr}}
=
P_0+(P_{100}-P_0)\frac{A_{i,j}}{100}
$$

设 \(p_{i,j}\) 为居民对职业 \(j\) 的熟练度，\(B_j\) 为满熟练度加成：

$$
S_{i,j,\mathrm{prof}}
=
B_j\frac{\operatorname{clamp}(p_{i,j},0,p_{\max})}{p_{\max}}
$$
...
\max\left(
0.25,\;
2.4\left(1-\frac{p}{100}\right)
\right)
$$

然后把结果限制在 0～100。生产使用成长前的熟练度，结算完成后才增加经验。典型初始熟练度约 16.7 时，约 22 个有效工作日达到 50、59 日达到 80、88 日达到 90、128 日达到 100。

有效工作日要求居民通过 `canResidentWork`，并且建筑存在劳动机会：

- 狩猎要求至少有 1 单位可用 HUNT；小数期望抽取尚未形成整抽也计入经验；
- 采矿要求至少一个有效链接矿区实际取得正数 ORE；
- 仓库拒收产物不影响经验；
- 每名居民对同一职业每天最多增长一次。
...
\operatorname{clamp}
\left(
S_{i,\mathrm{attr}}+S_{i,j,\mathrm{prof}},
S_{\min},
S_{j,\max}
\right)
$$
...
| 0 熟练度日增长 | `proficiencyGrowthAtZeroPerWorkday` | 2.4 |
| 最低日增长 | `minimumProficiencyGrowthPerWorkday` | 0.25 |

职业专属默认参数：

| 职业 | 健康/精神/力量/智力权重 | 满熟练度加成 | 最大生产力 |
|---|---|---:|---:|
| 狩猎 | 25 / 20 / 25 / 30 | 1.0 | 2.5 |
| 采矿 | 30 / 10 / 45 / 15 | 0.5 | 2.0 |
...
\operatorname{clamp}
\left(
0.5+\frac{A_{i,\mathrm{hunt}}}{100}+\frac{p_{i,\mathrm{hunt}}}{100},
0.5,
2.5
\right)
$$
...
\operatorname{clamp}
\left(
0.5+\frac{A_{i,\mathrm{mine}}}{100}+0.5\frac{p_{i,\mathrm{mine}}}{100},
0.5,
2.0
\right)
$$
...
| 对应职业加权属性 | 熟练度 | 狩猎生产力 | 狩猎期望物品/天 | 采矿生产力 | 采矿物品/天 |
|---:|---:|---:|---:|---:|---:|
| 0 | 0 | 0.5 | 0.875 | 0.5 | 1.75 |
| 50 | 0 | 1.0 | 1.75 | 1.0 | 3.5 |
| 50 | 16.7 | 1.167 | 2.042 | 1.083 | 3.792 |
| 50 | 50 | 1.5 | 2.625 | 1.25 | 4.375 |
| 50 | 80 | 1.8 | 3.15 | 1.4 | 4.9 |
| 100 | 100 | 2.5 | 4.375 | 2.0 | 7.0 |

多人基地分别把对应职业的居民生产力相加：

$$
\mathbb{E}[\text{狩猎物品/天}]
=1.75\sum_iS_{i,\mathrm{hunt}}
$$

$$
\text{采矿请求物品/天}
=3.5\sum_iS_{i,\mathrm{mine}}
$$
...
- `canResidentWork` 同时用于新工作分配和每日生产筛选；已在建筑居民集合中但当天不满足条件的居民不会贡献产量或获得经验。
...
- 力量和智力在初始化后固定；当前只有居民实际从事的对应职业熟练度会自动成长。
