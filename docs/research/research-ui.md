...
- Scope: Implemented research completion semantics, responsive drawing-desk archive, Phase 2 Knowledge Laboratory, full graph, project dialog, clue routing, and refresh boundaries
- Code anchors: [`Research`](../../src/main/java/com/teammoeg/frostedresearch/research/Research.java), [`ResearchData`](../../src/main/java/com/teammoeg/frostedresearch/data/ResearchData.java), [`TeamResearchData`](../../src/main/java/com/teammoeg/frostedresearch/data/TeamResearchData.java), [`DrawDeskScreen`](../../src/main/java/com/teammoeg/frostedresearch/gui/drawdesk/DrawDeskScreen.java), [`KnowledgeLabLayer`](../../src/main/java/com/teammoeg/frostedresearch/gui/knowledge/KnowledgeLabLayer.java), [`ResearchArchiveLayer`](../../src/main/java/com/teammoeg/frostedresearch/gui/archive/ResearchArchiveLayer.java), [`ResearchArchiveViewCache`](../../src/main/java/com/teammoeg/frostedresearch/gui/archive/ResearchArchiveViewCache.java), [`ResearchGraphViewport`](../../src/main/java/com/teammoeg/frostedresearch/gui/archive/ResearchGraphViewport.java), [`ResearchProjectWorkspace`](../../src/main/java/com/teammoeg/frostedresearch/gui/archive/ResearchProjectWorkspace.java), [`ResearchGraphLayoutEngine`](../../src/main/java/com/teammoeg/frostedresearch/gui/archive/graph/ResearchGraphLayoutEngine.java), [`CIconBatch`](../../src/main/java/com/teammoeg/chorda/client/icon/CIconBatch.java), `PanZoomViewport`, `UILayer`

## Current Player Surface

The drawing desk now has three independent surfaces: the original fixed-size desk, the existing full-window legacy research archive, and the full-window `KNOWLEDGE_LAB`. The Knowledge Laboratory is created lazily by `DrawDeskScreen#showKnowledgeLab`, hides container slots and external widgets like the archive, resizes with the window, and returns to the unchanged desk on Escape/back. Its compact desk entry is placed above the inventory region; the layer constructs its buttons before the immediate first resize, so opening it does not depend on a later CUI refresh.

`KnowledgeLabLayer` is a three-page archive rather than one geology worksheet. `OBSERVATIONS` exposes the complete newest-first, scrollable public observation list, a selected-record detail view, and the desk-local five-pin evidence board. `IDEAS` exposes every synchronized Idea, including visible `ORPHAN` state, its source/evidence counts, related method artifacts, and only the selected Idea's executable actions. `RESULTS` exposes acquired Finding, Design, Construction, Procedure, and Prototype categories; persistent result IDs whose current definition is missing stay visible as explicit orphans. The flat `KnowledgeLabButton` renderer uses one high-contrast, solid geometry for both drawing and hit testing, so long action text is clipped with a tooltip instead of being obscured by the old diagonal desk-button texture.

`KnowledgeLabLayout#calculate` is the single responsive geometry source for header, list, detail, and context columns. List scrolling addresses every entry in the synchronized `KnowledgeLabProjection`; render code does not reposition controls. A loaded topic's title, completion recipe, target Finding, or protocol checklist is still not rendered merely because that topic exists.

Record pins and organize/candidate/protocol/accept/clear operations use `FHDrawingDeskOperationPacket`. The server rechecks the open menu, loaded block entity, dimension, eight-block distance, team ownership, packet shape, record existence, five-pin limit, candidate index, current pins, Idea state, registered handler, and resolution. `DrawingDeskTileEntity.InspirationStatus` synchronizes a topic-neutral result for organize requests: no rule match, compatible paper required, ink required, game started, Idea recorded, or multiple choices ready. Organizing with no rule match does not start the game or consume paper/ink. During `V2_INSPIRATION`, the drawing-desk upper-left banner displays the active evidence count. The player may open the laboratory to review the frozen session and return to the same card board; the laboratory no longer forces itself closed from `tick`. Pins cannot change while that game is active, and “Cancel session” is the explicit destructive session action. After completion, one surviving rule match records or merges its `IdeaRecord` immediately; only genuinely ambiguous multiple matches require a candidate choice.
