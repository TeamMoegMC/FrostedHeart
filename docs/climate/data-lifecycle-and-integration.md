...
2026-08-24 的旧区块热区 JMH/JFR/JOL 数字只保留在对应 diary 作为迁移前历史证据。旧 fixture、专属 JMH 和 Gradle task 已随运行时代码删除，不能再作为当前性能入口。现行证据入口为 Phase A 与 published-air/完整管线任务。
...
新热学架构目前包含 gated Phase 0 probe、Phase A geometry/profile、Phase B/C Page 与 regular/mixed Air Mesh、Phase D arena-native solver/source、Phase E guard、Phase F Minecraft topology、Phase G Campfire/Generator/Radiator/Fountain sources、Phase H/I/J 可选材料/相变/辐射，以及 Phase K/L consumers/diagnostics。玩家空气/直接 source 辐射、publication 完整命中的作物和住宅/狩猎建筑，以及 admitted Page 内可编译的热侧物态转换已进入存档内实机测试权威；普通机器没有现存温度 consumer，非相变材料 profile 仍保持 dormant。旧区块热场 capability、形状类、失效包和专属基准已经删除；`PhaseZeroThermalRouting` 与 `ThermalBenchmarkEvidence` 只保留在 test source set，不能作为 production route 或 benchmark report 的代码锚点。

`LevelChunkSectionMixin_ThermalInput` 始终应用，只给 section 增加真实 `MinecraftThermalSectionAttachment` owner 和低层 mutation hook。`LevelChunkSectionMixin_Phase0aMutationProbe` 仅在 `frostedheart.phase0aMutationProbe=true` 的 Forge GameTest 运行中应用；普通游戏没有 Phase 0 owner 或 per-section counter。首次玩家温度查询为该 level 创建 `MinecraftThermalInput`；只自动信任 `hasDynamicShape=false` 的 state-static resolver 输出，并在玩家 section 缺失时对已加载 chunk capture 一个直接 Page。地下开放面可再 capture 一层已加载相邻 Page；该过程只用 `getChunkNow`、不递归，每维度最多保留 `64` 个 continuation-owned Page。辐射遮挡不走热签名解析：DDA 直接读取已加载 section 的 `BlockState`，动态形状按空气，静态状态只按 `BlockState.canOcclude()` 判断透明/整块不透明；不执行逐模组检查或部分形状栅格化。section mutation 随后经 owner 进入增量 invalidation。chunk unload 会撤销该 chunk 的 Page，服务端停止时 `closeAll` 释放 runtime/coordinator。

`MinecraftThermalEvents` 在 level tick 末驱动 gameplay cadence。稳定运行只在距上次 seal 达到 `5 ticks` 时封存 frame；Page admission/retirement、chunk lifecycle、geometry mutation、FarField/natural cut 变化以及 physical source 注册、移除、启停或功率变化会设置 urgent，并允许最新 frame 在不足 5 ticks 时立即启动。source timeline 仍按每个事件自己的 authoritative tick 精确积分 `P*dt`，求解合并不会丢中间能量。无 source power/pending energy 且残差连续稳定后，维度停止 seal/dispatch/republish，下一次 input event 唤醒。`topologyResolved=false` 继续保留 degraded query/fallback 语义，但等待未来外部证据本身不再阻止休眠。

frame 通过 `enableDispatch` 注册的 `Executor` 进入 topology apply、coordinator request、solve 与 publication。当前 gameplay 接线使用 `Runnable::run`，因此 tick 末仍在服务端主线程顺序完成整条路径，避免 Page admission 和 arena writer 并发。`Executor` 是保留给未来异步实现的唯一调度边界；替换者必须对同一 input 串行、按提交顺序且不重叠执行。没有 geometry、Page lifecycle、材料或外部边界变化的 frame 只 ACK 新 watermark 并复用已安装的 `ThermalSweep`；dirty 标志在 drain 时直接汇总，不再为稳定 frame 扫描全部 Page。每个稳定 Page 只常驻 `appliedSignatureIds[4096]`；`desiredSignatureIds` 在首次 dirty 时复制或接管 full-resync snapshot，commit 后直接转为 applied 并释放引用，约省 `16 KiB/Page`。`QueryPublication` gameplay 初始容量为 `256` 槽，发布前按 arena high-water mark 倍增；扩容仍在旧 backing 计费时申请完整新 reservation，失败时查询走现有 natural fallback。`65,536` 是 active-cell hard cap，不再是每维度 publication 的固定预留。
...
Phase L 不再在 production input 中累计 player/machine/crop/town legacy/new 误差、lookup 原因、seal/worker 纳秒、mailbox 状态或 last-report 快照；这些数据没有命令、HUD 或日志消费者，却会扩大每维度常驻状态。底层 `DimensionThermalRuntime.Diagnostics` 仍保留给定向测试和 JMH retained-size：logical writer 空闲时读取 arena、sweep 与 publication footprint；writer 正在修改状态时立即返回 `writerBusy=true`，并以 `-1` 表示不可读取的瞬时计数。真实性能证据继续由外部 JFR/JMH 和完整 workload gate 负责。

Phase L synthetic query diagnostics 由 `thermalPhaseLQueryDiagnostics` 统一生成 `build/reports/thermal-phase-l/` 下的 JMH JSON/text、100-receiver JFR、JOL retained-size 和环境/限制 manifest。`ThermalPublishedAirQueryFixtures` 直接组合 production `ThermalPage`、`ThermalCellArena`、`DimensionThermalRuntime` 与 `QueryPublication`，分别测一个共享 Page 和每个 receiver 分散到不同 Page 的 `1/10/50/100` 顺序查询批次。诊断首轮发现 `Map<Long, ThermalPage>` 的 section-key 装箱在分散布局中约分配 `24 B/query`；production `pages` 索引改为 fastutil `Long2ObjectOpenHashMap` 后，最终 JMH 的两种布局都约为 `0.01 B/query` 测量噪声。最终 100-query 批次均值约 `4.9 us`，p95 为 `5.0 us`，p99 为 `12.69 us`（共享）/`13.30 us`（分散）。JOL isolated graph 为共享 100 receivers `11,264 B`，分散 100 Pages/Cells `313,472 B`。这些数字不包含真实 `ServerPlayer`、`ServerLevel`、radiation receiver cache、topology rebuild、worker solve、TPS、chunk lifecycle 或生产模组集合，因此只能证明 hot query 和 isolated retained slope，不能批准 Phase L 或切换 gameplay authority。
...
最近一次已完成的 thermal 验证为 JUnit `240/240`、Forge GameTest `20/20` required。当前源码又增加一条 Fountain/Radiator profile JUnit，并把既有 physical-source GameTest 扩为四设备单一账本；本轮重跑被工作树中无关的 `FHTags.Blocks.SLUDGE` 改名引用阻断，因此不把新增覆盖写成已通过。thermal GameTest 路径包括 Phase 0a `7` 条、Phase 0b `1` 条、Phase A `5` 条、PR8 input、Phase G physical source、Phase J radiation、analytic compositor 和 underground continuation。Phase K 的 player/crop/town query 覆盖 mixed Brick、solid microcell、未 admission Page、publication age、真实 plant-status observation、town weighted-group compression、10,000 crop miss、4,096 town group miss 与只读 runtime state。`TownThermalProjectionTest` 另锁定负坐标 Brick 分组和与遍历顺序无关的代表点。Phase A census 枚举 `84,147` 个启用 BlockState：`82,197` 个走 generic static、`1` 个走 explicit、`12` 个走 contextual，合计 `82,210` resolved；`1,925` 个未注册 dynamic state 为 `NOT_REGISTERED`，另有 `12` 个 moving-piston state unresolved。共有 `262` 个完整 signatures、`259` 个唯一 geometry patterns、`2` 个 contextual outputs，最大观测 local regions 为 `4`。真实 Create bearing 测试冻结的语义是：assemble 捕获 `stone -> air`；contraption 移动期间按空气处理且不产生热几何 delta；disassemble 在目标位置捕获 `air -> stone`。因此当前方案没有 Create movement adapter、Mixin 或 AABB exclusion。真实 chunk lifecycle 测试通过 region ticket 驱动 load -> unload -> reload，并验证旧 section identity、generation 和 publication 被拒绝。
...
Phase 0a 现按冻结的常见路径范围完成，但只作为验证资产：Vanilla/Forge 常规 setter、流体、门、活塞、递归 mutation、Create assemble/disassemble，以及 Frosted Heart 已知 direct bypass 已有 GameTest 证据。真实 production hook 由 `LevelChunkSectionMixin_ThermalInput`、`MinecraftThermalEvents`、`MinecraftThermalInput.onRawBlockContainerReplaced` 和 `onSectionIdentityReplaced` 直接负责；`DebugCommand restore_backup` 执行 owner rebind + full resnapshot，FastNoise 只通知真实 raw block-container invalidation。Phase 0 reference、writer census、events 和 Forge GameTest 类从 production/deobf JAR 排除，仍保留在 main output 供 `runGameTestServer` 使用；probe Mixin 默认不应用。未知第三方 bypass 在玩家报告可复现后补专用 adapter。`/resetchunks` 是延期兼容项；以后若支持，应丢弃刷新区块的整份旧 thermal Page 并按 interest 懒重建，而不是保留旧状态。

尚无直接自动化覆盖：真实玩家加入后首次 profile/Page 构建、完整五部位长期推进、跨 section/chunk 移动、多人主线程耗时、recipe `/reload` 后 profile cut、实际 `ServerLevel.tickChunk` Mixin 等价性、热网多 provider/consumer tick、四种 physical machine source 的长期生命周期和红外网络频率。现有 JMH/JFR 不能替代存档内玩家测试；非相变材料/换热校准仍需继续。
