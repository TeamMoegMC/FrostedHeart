...
frame 通过 `enableDispatch` 注册的串行 `Executor` 进入 topology apply；返回 `APPLIED` 或 `DUPLICATE` 后，该维度的 `DimensionThermalRuntime.runOne` 至多执行一次并发布结果。当前 gameplay 接线使用 `Runnable::run`，因此 tick 末仍在服务端主线程顺序完成整条路径，避免 Page admission 和 arena writer 并发。`DimensionThermalRuntime` 自己拥有 latest-only scheduler 和 logical-writer gate，不再套第二层跨维度 mailbox/coordinator。执行器拒绝任务时 latest frame 仍留在 scheduler，并标记下一次 urgent seal 重试，不在调用线程并发回退。`Executor` 是保留给未来异步实现的唯一调度边界；替换者必须对同一 input 串行、按提交顺序且不重叠执行。没有 geometry、Page lifecycle、材料或外部边界变化的 frame 只 ACK 新 watermark 并复用已安装的 `ThermalSweep`；dirty 标志在 drain 时直接汇总，稳定 frame 在创建 active Page 列表和 section 索引之前返回。真实 captured Page 以空 unpublished Page admission，直接建立 `64` 个 world-aligned `4^3` arena fragment；不存在临时或常驻 coarse Page。稳定 Page 只替换 `dirtyBrickMask` 命中的 fragment，并按同一局部 overlap 迁移空气、材料 pole 与 phase reservoir 的 `H`。full-resync 必须捕获 `4096` 个 signature，但先按 topology semantics 汇总 64-bit Brick diff；相同快照不替换 slot，变化快照只重编命中的 Brick。未变 Brick 的 slot 不移动。每个 Brick 缓存自己作为世界负轴侧拥有者的 `+X/+Y/+Z` Air pair；geometry 变化只重编本 Brick 和三个负方向接口所有者，相邻 support 通过 arena support 原点 O(1) 定位真实 owner fragment。材料候选在 geometry 的同一次 64-block signature 遍历中收集，接触描述只刷新本 Brick 与六个面邻居；Page admission/retirement 才传播整面 section dependency。FarField 与 continuation 检查直接遍历每个开放面的 `16^2 = 256` 个边界方块，不再为每个面过滤完整 `16^3` Page。旧 fragment 只有在 replacement sweep 成功安装后才释放，retirement 会释放该 Page 的全部 fragment。每个稳定 Page 只常驻 `appliedSignatureIds[4096]`；`desiredSignatureIds` 在首次 dirty 时复制或接管 full-resync snapshot，commit 后直接转为 applied 并释放引用，约省 `16 KiB/Page`。`QueryPublication` gameplay 初始容量为 `256` 槽，发布前按 arena high-water mark 倍增；扩容仍在旧 backing 计费时申请完整新 reservation，失败时查询走现有 natural fallback。`65,536` 是 active-cell hard cap，不再是每维度 publication 的固定预留。

玩家测试 runtime 会启用 `MinecraftPhysicalSourceManager`、`RadiationService` 和 open-space FarField profile。开放空气阻抗复用同一组空气参数，外部温度仍按各维度的 `WorldTemperature.naturalAir` 计算；全局 `0..100` 风力把导纳连续缩放为 calm 值的 `1.0..1.8` 倍，不建立风力 profile 矩阵。physical manager 对普通设备变化只刷新对应 Campfire、Generator、Radiator 或 Fountain source，并保留固定 cold Page 上限；Page topology 失效时直接标记有上限的 source 集，每个 port 通过 `installedActiveBySection` 定位已安装 Page，不维护 source-to-section 反向索引。首次 cold Page admission 仍由 topology generation fallback 触发 degraded source 重绑，避免 source 永久停留在 loss sink。blocked/unloaded/unresolved port 继续走既有显式 sink，不产生第二份 `H/C` 或解析设备场。radiation 的 `128 sources / 1,024 sections / 128 receivers / top-8 / 24 rays` 上限一次性保留约 `729,408 B` optional memory per dimension。

`MinecraftThermalTopologyApplier` 保存一份 primitive Air adjacency 与 `componentBySlot`，它只是已编译 Air pair 的局部连通索引，不拥有第二份 `H/C` 或几何权威。Page admission/retirement、installed fragment 布局变化及全局 FarField 输入变化允许完整重建；普通方块 mutation 只删除和安装 dirty pair fragment 的 edge。开口只合并两个端点 component；闭口只在被触及的旧 component 内 flood-fill 以识别 split，未触及 component 不遍历、不重分类也不重写 boundary。开放 patch 的 slot/count/sky 临时数组只在编译一个 Brick 时存在，图中常驻实际 cell 计数，而 Page 每 Brick 只另存一个 unresolved patch 整数。Page capture 同时保存 `16x16` 字节高度图截面；缺邻居 Page 的开放 microface 按 cell 累计。topology applier 只在 component 已证明真实天空暴露、环境存在对应 `FarFieldProfileRegistry.Profile` 且当前 source power/温差位于 `ApplicabilityDomain` 时安装 open-space boundary；开放方向数本身不是室外证据。其余开放 component 保持 unresolved，不生成未校准 transport。Page 的自然边界温度来自不含 mesh、analytic field 和随机噪声的 `WorldTemperature.naturalAir`；每 `200` ticks 刷新自然温度、天空截面和风力，背景变化不足 `0.25 degC`、风力导纳变化不足 `0.05` 时不替换 sweep。

`ThermalSweepFragments` 是唯一的 production solve layout，按 Brick 保存 primitive Air pair、材料 contribution、材料 boundary 和 phase contact；FarField boundary 按 arena slot 保存。`ThermalSweep` 只保留 operation/result API、pending patch 和对 fragment layout 的执行委托，不再保存或执行第二套 flat operation 数组。普通 topology commit 只构造一个包含 dirty fragment 与受影响 FarField slot 的 patch，并在 `DimensionThermalRuntime.finishTopologyUpdate` 持有 logical writer 时原子提交；不会重新创建全局 operation 数组，也不会扫描全部 active Page 或 `arena.highWaterMark()`。材料 contribution 固定按 fragment index、fragment 内 operation index 聚合，相同最终 topology 因此不再依赖 mutation 历史；forward/reverse、boundary/phase 次序、补偿总和与 stale-slot 检查不变。Page 生命周期或 fragment 总数变化时才建立新的完整 fragment table。材料 surface、phase surface、stateless bridge 及 material-pole/phase-reservoir slot index 都按 Brick 保存，Page 不再维护材料 traversal/rank 镜像。

`MinecraftThermalTopologyApplier` 只用一个 identity-based dirty Page 集合表示待处理 Page；geometry、pair owner 和 material neighbor 的差异仍由 Page-local Brick mask 表达。`fullTopologyCompilationRequired` 只保留给生命周期和全局输入，普通增量 commit 每个 dirty Page 只分派一次。线程外 mutation drain 先比较现有 dimension revision；revision 未推进时直接返回，不遍历全部 attached section owner，revision 推进时仍使用 owner 的临时精确 bitset 处理并发位置。
...
`DimensionThermalRuntime` 的 latest-only scheduler 是每维度唯一求解调度状态；`MinecraftThermalInput` 只保留一个要求串行执行的 `Executor` 边界，当前 `Runnable::run` 不需要跨维度 ready queue、fairness mailbox 或第二份 lifecycle state。sleep 只要求整个 aggregate solve set 无 source power/pending energy、无 pending frame且 pair/boundary residual 稳定；unresolved frontier 是结果完整性状态，不是持续工作队列。未来若接真实异步 pool，backpressure 与跨维度公平性必须基于实测重新设计，不能把已删除的同步占位 coordinator 当作现有保证。Minecraft geometry、topology、四类 physical source、player/radiation、crop、town query、三后端 compositor 与 recipe-compiled hot-side phase 已进入上述实机测试路径；其他材料 profile 仍 dormant，普通机器没有真实温度 consumer 或预留 observation API。尚未完成的是非相变材料校准、冷侧 phase authority、真实多人 workload 和 gameplay 数值冻结。
